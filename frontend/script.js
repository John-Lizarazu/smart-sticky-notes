...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderGroupedAndUngrouped() {
    ...
    if (groupedCategories?.length) {
      ...
      groupedCategories.forEach((cat) => {
        ...
        cat.notes.forEach((noteText) => {
          ...
          card.onclick = () => deleteNoteByText(noteText);
    ...
    const groupedTexts = new Set(
      groupedCategories?.flatMap((c) => c.notes.map((t) => t.toLowerCase())) || []
    );
    const ungroupedNotes = notes.filter((n) => !groupedTexts.has(n.text.toLowerCase()));
  ...
  // === DELETE NOTE BY TEXT (for grouped) ===
  function deleteNoteByText(text) {
    // remove from grouped
    groupedCategories.forEach((cat) => {
      cat.notes = cat.notes.filter((t) => t !== text);
    });
    // remove empty categories
    groupedCategories = groupedCategories.filter((c) => c.notes.length > 0);
    renderGroupedAndUngrouped();
  }
  ...
  groupBtn.onclick = async () => {
    ...
    try {
      ...
      // Merge with existing grouped categories instead of replacing
      if (groupedCategories) {
        const existingTopics = new Map(
          groupedCategories.map((c) => [c.topic.toLowerCase(), c])
        );

        categories.forEach((newCat) => {
          const existing = existingTopics.get(newCat.topic.toLowerCase());
          if (existing) {
            // append new unique notes
            newCat.notes.forEach((n) => {
              if (!existing.notes.includes(n)) existing.notes.push(n);
            });
          } else {
            groupedCategories.push(newCat);
          }
        });
      } else {
